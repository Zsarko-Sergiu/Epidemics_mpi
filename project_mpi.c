...
void start_mpi(int sim_time, int N) {
    ...
    if (!local_people) {
        fprintf(stderr, "Rank %d: Failed to allocate memory for local people.\n", rank);
    ...
    for (int i = 0; i < size; i++) {
        send_counts[i] = base_people + (i < remainder ? 1 : 0);
        displs[i] = i * base_people + (i < remainder ? i : remainder);
    }
